...
✅ ALL BUGS FIXED | ✅ ENHANCED FEATURES | ✅ PRODUCTION READY


st.set_page_config(
    page_title="Amazon Ads Dashboard Pro v3.0",
    page_icon="🏢",
    layout="wide",
...
.agency-header{background:linear-gradient(135deg,#1e40af,#3b82f6);padding:2rem;border-radius:16px;margin-bottom:1.5rem;text-align:center;color:white;}
div[data-testid="stMetric"]{background:rgba(30,41,59,0.8);border:1px solid rgba(148,163,184,0.3);border-radius:12px;padding:1.5rem 1rem;}
div[data-testid="stMetricLabel"]{font-size:0.95rem !important;color:#cbd5e1 !important;white-space:normal !important;}
div[data-testid="stMetricValue"]{font-size:1.8rem !important;color:#fff !important;white-space:normal !important;word-break:break-word !important;}
...
class ClientData:
    def __init__(self, name, industry="E-commerce", budget=50000):
        ...
        self.target_acos, self.target_roas, self.target_cpa = None, None, None
...
class CompleteAnalyzer:
    ...
    def __init__(self, df, client_name, target_acos=None, target_roas=None, target_cpa=None):
        self.client_name, self.target_acos, self.target_roas, self.target_cpa = client_name, target_acos, target_roas, target_cpa
        self.df, self.error = None, None
        try:
            # CRITICAL: Deep copy to prevent data mixing
    ...
    def _validate_and_prepare_data(self, df):
        ...
        df = df.copy()  # Extra safety
        df.columns = df.columns.str.strip()
        mapping = {'customer search term':'Customer Search Term','search term':'Customer Search Term','keyword':'Customer Search Term','campaign':'Campaign Name','campaign name':'Campaign Name','ad group':'Ad Group Name','ad group name':'Ad Group Name','match type':'Match Type','matchtype':'Match Type','7 day total sales':'Sales','7 day total orders':'Orders','7 day orders':'Orders','total sales':'Sales','total orders':'Orders','sales':'Sales','orders':'Orders','cost':'Spend','spend':'Spend','impressions':'Impressions','clicks':'Clicks','cpc':'CPC','cost per click':'CPC'}
        ...
        for col in ['Spend','Sales','Clicks','Impressions','Orders','CPC']:
            if col in df.columns:
                if df[col].dtype == 'object': df[col] = df[col].astype(str).str.replace('[₹$,]','',regex=True).str.replace('%','').str.strip()
    ...
    def get_client_summary(self):
        try:
            ...
            return {'total_spend':ts,'total_sales':tsa,'total_profit':safe_float(self.df['Profit'].sum()),'total_orders':to,'total_clicks':tc,'total_impressions':ti,'total_wastage':tw,'roas':(tsa/ts if ts>0 else 0),'acos':(ts/tsa*100 if tsa>0 else 0),'avg_cpc':avg_cpc,'avg_ctr':avg_ctr,'avg_cvr':avg_cvr,'avg_cpa':avg_cpa,'conversion_rate':(to/tc*100 if tc>0 else 0),'keywords_count':len(self.df),'campaigns_count':safe_int(self.df['Campaign Name'].nunique())}
        except Exception as e:
            print(f"Summary error: {e}")
            return self._empty()

    def _empty(self):
        return {k:0 for k in ['total_spend','total_sales','total_profit','total_orders','total_clicks','total_impressions','total_wastage','roas','acos','avg_cpc','avg_ctr','avg_cvr','avg_cpa','conversion_rate','keywords_count','campaigns_count']}
    ...
    def get_performance_insights(self):
        ...
        insights = {'ctr_insights':[],'cvr_insights':[],'roas_insights':[],'cpa_insights':[],'content_suggestions':[]}
        try:
            avg_ctr = summary['avg_ctr']
            if avg_ctr < 0.5:
                insights['ctr_insights'].append({'level':'critical','metric':'CTR','value':f'{avg_ctr:.2f}%','issue':'Extremely low - ads not attractive','action':'URGENT: Revamp ad copy and targeting'})
                insights['content_suggestions'].extend(['🎯 Add power words: "Best", "Top", "Premium"','💰 Highlight pricing: "50% Off", "Free Shipping"','⭐ Add ratings: "4.5★ Rated", "Bestseller"','🎁 Create urgency: "Limited Time", "Today Only"','📸 Use lifestyle product images'])
            elif avg_ctr < 1.0:
                insights['ctr_insights'].append({'level':'warning','metric':'CTR','value':f'{avg_ctr:.2f}%','issue':'Low - needs improvement','action':'Improve ad copy and images'})
                insights['content_suggestions'].extend(['📝 A/B test different headlines','✨ Add product benefits in title','🏆 Highlight unique selling points'])
            elif avg_ctr >= 3.0:
                insights['ctr_insights'].append({'level':'success','metric':'CTR','value':f'{avg_ctr:.2f}%','issue':'Excellent!','action':'Keep current strategy'})

            avg_cvr = summary['avg_cvr']
            if avg_cvr < 1.0:
                insights['cvr_insights'].append({'level':'critical','metric':'CVR','value':f'{avg_cvr:.2f}%','issue':'Poor conversion','action':'Fix product page and pricing'})
                insights['content_suggestions'].extend(['📄 Optimize product descriptions','💵 Review pricing vs competitors','⭐ Get more customer reviews','📦 Highlight free/fast shipping','🎨 Add A+ content'])
            elif avg_cvr < 3.0:
                insights['cvr_insights'].append({'level':'warning','metric':'CVR','value':f'{avg_cvr:.2f}%','issue':'Below average','action':'Optimize product pages'})

            roas = summary['roas']
            if roas < 1.0:
                insights['roas_insights'].append({'level':'critical','metric':'ROAS','value':f'{roas:.2f}x','issue':'Losing money','action':'PAUSE and fix fundamentals'})
            elif roas < 2.0:
                insights['roas_insights'].append({'level':'warning','metric':'ROAS','value':f'{roas:.2f}x','issue':'Low profitability','action':'Reduce poor performer bids'})
            elif roas >= 3.0:
                insights['roas_insights'].append({'level':'success','metric':'ROAS','value':f'{roas:.2f}x','issue':'Great!','action':'Scale winning campaigns'})

            avg_cpa = summary['avg_cpa']
            if self.target_cpa and avg_cpa > self.target_cpa:
                insights['cpa_insights'].append({'level':'warning','metric':'CPA','value':format_currency(avg_cpa),'issue':f'Above target {format_currency(self.target_cpa)}','action':'Reduce bids or improve CVR'})
    ...
    def classify_keywords_improved(self):
        ...
        try:
            if self.df is None: return cats
        ...
        try:
            ...
            for _, r in self.df.iterrows():
                try:
                    ...
                    if c>=3 and sp<150:
                        if o==1:
                            fk.append({'Keyword':safe_str(r.get('Customer Search Term')),'Match Type':safe_str(r.get('Match Type')),'Clicks':c,'Orders':o,'Spend':format_currency(sp),'Status':'🟡 Promising','Action':'1 order, monitor','Recommendation':'Keep current bid'})
                        elif c>=5 and o==0:
                            fk.append({'Keyword':safe_str(r.get('Customer Search Term')),'Match Type':safe_str(r.get('Match Type')),'Clicks':c,'Orders':o,'Spend':format_currency(sp),'Status':'⚪ Watching','Action':'Relevant, needs time','Recommendation':'Give more data'})
                except: continue
            return fk
        except: return []
    ...
    def get_match_type_strategy(self):
        ...
        try:
            ...
            for mt in ['EXACT','PHRASE','BROAD']:
                ...
                if len(md)>0:
                    ts,tsa,to = md['Spend'].sum(),md['Sales'].sum(),md['Orders'].sum()
                    ro = (tsa/ts) if ts>0 else 0
                    ac = (ts/tsa*100) if tsa>0 else 0
                    cv = (to/md['Clicks'].sum()*100) if md['Clicks'].sum()>0 else 0
                    s['current_performance'][mt] = {'spend':ts,'sales':tsa,'roas':ro,'orders':to,'acos':ac,'cvr':cv,'keywords':len(md)}
                    if mt=='EXACT' and ro>=3.0:
                        s['recommendations'].append({'match_type':'EXACT','action':'✅ Scale aggressively','reason':f'High ROAS {ro:.2f}x','priority':'HIGH'})
                    elif mt=='PHRASE' and ro>=2.0:
                        s['recommendations'].append({'match_type':'PHRASE','action':'⚡ Test & optimize','reason':f'Good ROAS {ro:.2f}x','priority':'MEDIUM'})
                    elif mt=='BROAD' and ro<1.5:
                        s['recommendations'].append({'match_type':'BROAD','action':'⚠️ Reduce/pause','reason':f'Low ROAS {ro:.2f}x','priority':'HIGH'})
            return s
        except Exception as e:
    ...
    def get_roas_improvement_plan(self):
        ...
        if wc>0:
            ...
            p['immediate_actions'].append({'priority':'🚨 URGENT','action':f'Pause {wc} wastage keywords','impact':f'Save {format_currency(ws)}/month','how':'Exports → Negatives'})
        hp = len(c['high_potential'])
        if hp>0:
            p['short_term'].append({'priority':'🏆 HIGH','action':f'Scale {hp} winners','impact':'Sales +20-30%','how':'Bids +15-25%'})
        if cr<1.0:
            p['immediate_actions'].insert(0,{'priority':'🚨 CRITICAL','action':'PAUSE ALL','impact':'Stop losing money','how':'Fix product/pricing'})
        p['short_term'].append({'priority':'⚡ MEDIUM','action':'Optimize listings','impact':'CVR +50-100%','how':'Better images, A+'})
        p['long_term'].append({'priority':'📊 ONGOING','action':'Test new keywords','impact':'Find winners','how':'Weekly review'})
        return p
            ...
            for _, r in self.df.iterrows():
                try:
                    ...
                    if ro>=3.5 and cv>=2.0 and o>=2:
                        nb=cpc*1.25
                        s.update({'Action':'INCREASE','Suggested Bid':format_currency(nb),'Change (%)':25,'Reason':f"Champion {ro:.2f}x"})
                    elif ro>=tr and cv>=1.0 and o>=1:
                        nb=cpc*1.15
                        s.update({'Action':'INCREASE','Suggested Bid':format_currency(nb),'Change (%)':15,'Reason':'Above target'})
                    elif sa==0 and sp>=100:
                        s.update({'Action':'PAUSE','Suggested Bid':'₹0.00','Change (%)':-100,'Reason':f"₹{sp:.0f} wasted"})
                    elif ro<1.5 and sp>=50:
                        nb=cpc*0.7
                        s.update({'Action':'REDUCE','Suggested Bid':format_currency(nb),'Change (%)':-30,'Reason':f"Poor {ro:.2f}x"})
                    elif ac>ta and sp>=50:
                        red=min(30,(ac-ta)/ta*100)
                        nb=cpc*(1-red/100)
                        s.update({'Action':'REDUCE','Suggested Bid':format_currency(nb),'Change (%)':-int(red),'Reason':f"ACOS {ac:.1f}% high"})
                    else: continue
                    sug.append(s)
                except: continue
...
def render_sidebar():
    with st.sidebar:
        ...
        with st.expander("➕ Add Client"):
            ...
            with c1: tacos = st.number_input("ACOS%", value=0.0, step=5.0, key="add_acos")
            with c2: troas = st.number_input("ROAS", value=0.0, step=0.5, key="add_roas")
            with c3: tcpa = st.number_input("CPA ₹", value=0.0, step=50.0, key="add_cpa")
                ...
                if not nm:
                    ...
                    try:
                        with st.spinner(f"Analyzing {nm}..."):
                            # CRITICAL: Read fresh data for each client
                            df = pd.read_excel(up)
                            ...
                            # Create analyzer with ISOLATED data
                            cd.analyzer = CompleteAnalyzer(
                                df.copy(),  # CRITICAL: Pass copy
                                nm,
                                cd.target_acos,
                                cd.target_roas,
                                cd.target_cpa
...
def render_keywords_tab(an):
    try:
        ...
        # Summary metrics
        c1,c2,c3,c4,c5 = st.columns(5)
        with c1: st.metric("🏆 Scale", len(c['high_potential']))
        with c2: st.metric("⚡ Test", len(c['opportunities']))
        with c3: st.metric("👀 Watch", len(c['future_watch']))
        with c4: st.metric("⚠️ Reduce", len(c['low_potential']))
        with c5: st.metric("🚨 Pause", len(c['wastage']))
...
def render_bid_tab(an):
    try:
        ...
        if sug:
            ...
            st.markdown("---")
            c1,c2,c3 = st.columns(3)
            with c1: st.metric("⬆️ Scale Up", inc)
            with c2: st.metric("⬇️ Reduce", red)
            with c3: st.metric("⏸️ Pause", pau)

            st.markdown(f"---\\n**Showing {len(filt)} of {len(sug)} suggestions**")
...
def render_all_clients_tab():
    try:
        ...
        if data:
            ...
            with c1: st.metric("Total Clients", len(data))
            with c2: st.metric("🟢 Healthy", len([d for d in data if '🟢' in d['Status']]))
            with c3: st.metric("🟡 Okay", len([d for d in data if '🟡' in d['Status']]))
            with c4: st.metric("🔴 Attention", len([d for d in data if '🔴' in d['Status']]))
        else:
